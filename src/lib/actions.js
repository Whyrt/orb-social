...
export function useActions() {
    ...
    const loadUserData = async (nickname, attempt = 1) => {
        ...
        try {
            ...
            const [memberRes, friendsRes, invitesRes] = await Promise.all([
                ...
                supabase.from('friends').select('*').or(`user1_nickname.eq.${nickname},user2_nickname.eq.${nickname}`),
            ...
            console.error(`User Load error (attempt ${attempt}/${MAX_LOAD_ATTEMPTS}):`, error);
    ...
    const handleSignalStart = async (amountStr) => {
        ...
        try {
            ...
            console.error('Signal start error:', error);
    ...
    const handleSignalCashout = async () => {
        ...
        try {
            ...
            console.error('Signal cashout error:', error);
        }
