        ...
        <div className="absolute inset-0 z-20 flex flex-col h-[100dvh] overflow-hidden grid-pattern"
            ...
            {!activeChat ? (
                ...
                <div {...bindListSwipes()} className="flex flex-col h-full pt-14 px-4 pb-24 overflow-hidden animate-block">
                    ...
                    <div className="flex flex-col gap-2 overflow-y-auto no-scrollbar pb-8">
                        ...
                        <div onClick={() => setActiveChat('global')}
                            className="group flex items-center justify-between p-4 terminal-border transition-all cursor-pointer"
                            style={{ backgroundColor: 'var(--card-bg)' }}>
                            <div className="flex items-center gap-3">
                                <div className="w-10 h-10 terminal-border flex items-center justify-center text-base">🌐</div>
                                <div>
                                    <h3 className="font-mono text-xs uppercase tracking-widest" style={{ color: 'var(--foreground)' }}>Global Node</h3>
                                    <p className="text-[6px] font-mono uppercase tracking-widest mt-0.5" style={{ color: 'var(--foreground-dim)' }}>Public Channel</p>
                                </div>
                            </div>
                        ...
                        {/* Section Header */}
                        <p className="text-[6px] font-mono uppercase tracking-widest ml-2 mt-4 mb-2" style={{ color: 'var(--foreground-dim)' }}>Private Links</p>

                        {/* Empty State */}
                        {friends.length === 0 && (
                            <p className="text-[8px] font-mono ml-2 italic" style={{ color: 'var(--foreground-dim)' }}>No active links.</p>
                        )}
                        ...
                        {friends.map((f, i) => (
                            <div key={f}
                                ...
                                className="flex items-center justify-between p-4 terminal-border transition-all cursor-pointer animate-block"
                                style={{ 
                                    backgroundColor: 'var(--card-bg)',
                                    animationDelay: `${i * 0.05}s`
                                }}>
                                <div className="flex items-center gap-3">
                                    <div className="w-10 h-10 terminal-border flex items-center justify-center font-mono text-sm"
                                         style={{ color: 'var(--foreground)' }}>
                                        {f[0].toUpperCase()}
                                    </div>
                                    <h3 className="font-mono text-xs uppercase tracking-widest" style={{ color: 'var(--foreground)' }}>@{f}</h3>
                                </div>
                ...
                <div className="flex flex-col h-full relative" style={{ backgroundColor: 'var(--background)' }}>
                    {/* Chat Header */}
                    <div className="pt-12 pb-3 px-4 flex items-center justify-between z-[2100]"
                         style={{ 
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h3 className="font-mono text-xs uppercase tracking-widest" style={{ color: 'var(--foreground)' }}>@{activeChat}</h3>
                                <div className="flex items-center gap-1.5 mt-0.5">
                                    <span className="w-1.5 h-1.5 rounded-full animate-pulse" style={{ backgroundColor: 'var(--success)' }}></span>
                                    <span className="text-[5px] font-mono uppercase tracking-widest" style={{ color: 'var(--foreground-dim)' }}>Link Active</span>
                                </div>
                            </div>
                        </div>
                        <button 
                    ...
                    <div
                        ...
                        className="flex-1 overflow-y-auto px-3 py-3 space-y-2 no-scrollbar pb-36"
                    ...
                    >
                        ...
                        {!hasMoreMessages && currentMessages.length > 0 && (
                            <div className="text-center py-2">
                                <p className="text-[6px] font-mono uppercase tracking-widest" style={{ color: 'var(--foreground-dim)' }}>End of history</p>
                            </div>
                        )}
                    ...
                    {/* Reply Preview */}
                    {replyTo && (
                        <div className="flex justify-between items-center terminal-window p-2 mb-2 animate-block">
                            <div className="flex flex-col overflow-hidden">
                                <span className="text-[6px] font-mono uppercase tracking-widest mb-0.5" style={{ color: 'var(--foreground-dim)' }}>
                                    Replying to @{replyTo.sender}
                                </span>
                                <span className="text-[8px] truncate max-w-[180px]" style={{ color: 'var(--foreground-muted)' }}>{replyTo.text || 'Media node'}</span>
                            </div>
                            <button 
                                onClick={() => setReplyTo(null)} 
                                className="w-6 h-6 terminal-border flex items-center justify-center press-effect"
                                style={{ color: 'var(--foreground-muted)' }}
                            >
                                ✕
                            </button>
                    ...
                    {/* Message Input */}
                    <div className="absolute bottom-6 left-0 w-full px-3 flex flex-col z-50">
                        <div className="flex items-end gap-2">
                            <div className="flex-1 flex items-center px-3 py-2 gap-2 min-h-[46px] transition-all terminal-border"
                                 style={{ 
                                ...
                                <input
                                    ...
                                    className="flex-1 bg-transparent text-[10px] font-mono outline-none py-1 uppercase tracking-wider"
                                    style={{ 
                            ...
                            <button
                                ...
                                className="w-[46px] h-[46px] terminal-border flex items-center justify-center transition-all press-effect"
                                style={{
                                    backgroundColor: isRecording ? 'var(--accent)' : 'transparent',
                                    color: isRecording ? 'var(--accent-text)' : 'var(--foreground)'
                                }}
                            >
